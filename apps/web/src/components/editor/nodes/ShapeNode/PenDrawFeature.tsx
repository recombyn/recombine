...
import {
  ...
  withMirroredHandles,
  type PenAnchor,
} from '@/components/editor/nodes/ShapeNode/penPath';
...
/** Scene-space radius matching ~screenPx at current paper scale. */
function hitRadiusScene(
  paperEl: HTMLElement,
  artboard: { width: number; height: number },
  screenPx: number
) {
  const rect = paperEl.getBoundingClientRect();
  const scale = rect.width / Math.max(1, artboard.width);
 ...
 * click curved anchor to clear handles (corner), close near first, Enter / Esc.
 */
export default function PenDrawFeature({
  ...
  const finish = (closed: boolean) => {
    ...
    const pad = Math.max(1, Number(strokeWidth) || 2) / 2;
    const origin = {
  ...
  useEffect(() => {
    ...
    const radii = () => ({
      anchor: hitRadiusScene(paperEl, artboard, ANCHOR_HIT_PX),
      handle: hitRadiusScene(paperEl, artboard, HANDLE_HIT_PX),
    });
    ...
    const onDown = (e: PointerEvent) => {
      ...
      if (handleHit && (e.altKey || e.metaKey)) {
        ...
        setHoverHandle(null);
        setHoverAnchor(null);
        setPaperCursor(paperEl, 'crosshair');
        return;
      }
      ...
      if (handleHit) {
        ...
        setHoverAnchor(null);
        setPaperCursor(paperEl, 'grabbing');
      ...
      if (anchorIdx >= 0) {
        ...
        setCloseHot(false);
        setCursor(p);
        setPaperCursor(paperEl, 'pointer');
        return;
      }
    ...
    const onUp = (e: PointerEvent) => {
      ...
      if (dragKindRef.current?.kind === 'handle') {
        ...
        if (aIdx >= 0) {
          ...
          setHoverAnchor(null);
          setHoverHandle(hit);
          setPaperCursor(paperEl, hit ? 'grab' : 'crosshair');
    ...
    return () => {
      ...
      paperEl.style.cursor = '';
    };
  }, [enabled, paperEl, artboard, onCommit, onCancel, strokeWidth]);
  ...
  const anchorR = Math.max(3.5, sw / 2 + 1.5);
  const handleR = Math.max(2.5, Math.min(3.5, anchorR - 0.5));
