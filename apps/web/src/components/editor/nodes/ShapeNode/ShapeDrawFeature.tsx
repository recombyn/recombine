...
import { ARROW_HEAD, ptsAttr, shapeVertexPoints } from '@/store/scene/sceneShapes';
...
export default function ShapeDrawFeature({
  ...
  const session = useRef<{ x0: number; y0: number; x1: number; y1: number } | null>(null);
  ...
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      ...
      session.current = { x0: p.x, y0: p.y, x1: p.x, y1: p.y };
    ...
    const onMove = (e: PointerEvent) => {
      ...
      const p = clientToScene(paperEl, artboard, e.clientX, e.clientY);
      session.current.x1 = p.x;
      session.current.y1 = p.y;
      setPreview({ ...session.current });
    };
  ...
  }, [enabled, paperEl, artboard, shapeKind, onCreate]);
