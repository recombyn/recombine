...
export default function MultiSelectionToolbar({
  ...
  const runBoolean = (mode: BoolMode) => {
    ...
    attrs.closed = 'true';
    // Keep the primary shape's fill (solid / gradient / image) on the boolean result.
    if (sampleNode?.attrs && typeof sampleNode.attrs === 'object') {
      const src = sampleNode.attrs as Record<string, unknown>;
  ...
  // Fig.1 — selected group: 解除编组 | export (badge + chevron)
  if (groupId) {
    return (
      <SelectionToolbarShell box={box}>
            ...
            <ExportSelectionPopover
              ...
              showBadge
              showChevron
              triggerClassName="inline-flex h-8 items-center gap-0.5 rounded-lg px-2 text-[var(--ink)] transition-colors hover:bg-[var(--accent-soft)]"
            />
