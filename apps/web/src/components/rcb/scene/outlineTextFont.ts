...
import { loadFontCatalog, resolveFontFileUrl } from '@/components/rcb/scene/fontCatalog';
import { parseNodeText, parseNodeTextStyle, textVerticalOriginY, toFabricFontFamily } from '@/components/rcb/scene/sceneText';
import type { OutlineResult } from '@/components/rcb/scene/outlineToPath';
...
type FkFont = {
  ...
  descent: number;
  layout: (text: string) => {
    glyphs: Array<{ path?: { commands: FkCommand[] }; advanceWidth?: number }>;
  ...
  if (!pending) {
    pending = (async () => {
      try {
        ...
        return createFontkitFont(buf) as unknown as FkFont;
      } catch (err) {
...
function commandsToPathD(commands: FkCommand[], ox: number, oy: number, scale: number): string {
  ...
  const Y = (y: number) => oy - y * scale; // font space ↑ → SVG ↓
  for (const c of commands) {
    const a = c.args || [];
    switch (c.command) {
      case 'moveTo':
        if (a.length >= 2) out.push(`M ${X(a[0]).toFixed(2)} ${Y(a[1]).toFixed(2)}`);
        break;
      case 'lineTo':
        if (a.length >= 2) out.push(`L ${X(a[0]).toFixed(2)} ${Y(a[1]).toFixed(2)}`);
        break;
      case 'quadraticCurveTo':
        if (a.length >= 4) {
          out.push(
            `Q ${X(a[0]).toFixed(2)} ${Y(a[1]).toFixed(2)} ${X(a[2]).toFixed(2)} ${Y(a[3]).toFixed(2)}`
          );
        }
        break;
      case 'bezierCurveTo':
        if (a.length >= 6) {
          out.push(
            `C ${X(a[0]).toFixed(2)} ${Y(a[1]).toFixed(2)} ${X(a[2]).toFixed(2)} ${Y(a[3]).toFixed(2)} ${X(a[4]).toFixed(2)} ${Y(a[5]).toFixed(2)}`
          );
        }
        break;
      case 'closePath':
