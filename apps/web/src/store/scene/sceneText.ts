import { flatFontOptions } from './fontCatalog';
...
/** @deprecated Prefer fontCatalog tree; kept for flat consumers. */
export const FONT_OPTIONS = flatFontOptions();
...
export function isTextStrike(style: Partial<TextStyle> | null | undefined) {
  return String(style?.textDecoration || '').includes('line-through');
}
...
export function wrapPlainTextLines(
  ...
  const padX = 8;
  const limit = Math.max(fontSize, maxWidth - padX);
...
export function measurePlainTextSize(text: string, style: Partial<TextStyle> = {}) {
  ...
  const padX = 8;
  // Height follows CSS line boxes (n × fontSize × lineHeight). Extra padY used to
  // sit only under the ink and make the selection chrome look bottom-heavy.
  return {
    width: Math.max(24, Math.ceil(maxW + padX)),
    height: Math.max(
...
export function parseNodeTextStyle(attrs: Record<string, any> = {}): TextStyle {
  ...
  if (attrs.ORIGIN_DATA) {
    try {
      ...
      if (child?.strike || child?.strikethrough) style.textDecoration = 'line-through';
      if (child?.underline) {
...
export function buildTextAttrsPreservingMarkdown(
  ...
  return buildMarkdownTextAttrs(md, { ...parseNodeTextStyle(attrs), ...style });
}
