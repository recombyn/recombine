"""Lazy OpenCLIP encoder for layout / color / aesthetic towers.

Requires optional extras: pip install -e ".[aesthetics]"
"""
...
def clip_status() -> dict[str, Any]:
    return {
        ...
        "hint": (
            None
            if clip_available()
            else 'Install extras: pip install -e ".[aesthetics]" (torch + open-clip-torch + numpy)'
        ),
    }
