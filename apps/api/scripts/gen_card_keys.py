...
1. Random plaintext (xxxxx-xxxxx, ambiguous chars excluded)
2. Store SHA256(plaintext + CARD_KEY_SALT) in SQLite — never plaintext
...
Env:
  CARD_KEY_SALT must be set (same salt the API uses for redeem).
"""
...
def main() -> int:
    ...
    inserted = insert_card_keys(plaintexts=plaintexts, tokens=args.tokens, expires_at=expires_at)
